...
- `src/extension.ts` — activation entry point; registers commands and the sidebar view provider, opens the tree panel unconditionally on activate. Also registers an advanced, Command-Palette-only command and a URI handler (`vscode://agentharnesses.ahar-visualizer/openTree?...`) for opening additional independently-configured panels — see `references/multi-panel-testing.md`.
